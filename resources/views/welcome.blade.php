      ...
      <div class="relative flex items-top justify-center min-h-screen bg-gray-100 ligth:bg-gray-900 sm:items-center py-4 sm:pt-0">
          @if (Route::has('login'))
              <div class="hidden fixed top-0 right-0 px-6 py-4 sm:block">
                <a href="#" class="text-sm text-dark-700 underline">Home</a>&emsp;
                <a href="#" class="text-sm text-dark-700 underline">Daftar Vanue</a>&emsp;
                <a href="#" class="ml-4 text-sm text-dark-700 underline">Contact</a>&emsp;
                  @if(Auth::check() && Auth::user()->admin == 1)
                     <a href="{{ url('admin/dashboard') }}" class="text-sm text-dark-700 underline">Dashboard</a>
                  @elseif(Auth::check() && Auth::user()->admin != 1)
                      <a href="{{ url('/home') }}" class="text-sm text-dark-700 underline">Home</a>&emsp;
                  @else
